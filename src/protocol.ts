export const START = '<!-- ade:start -->';
export const END = '<!-- ade:end -->';
export const DEFAULT_CMD = 'npx awesome-delivery-engine';

export function protocolBody(cmd = DEFAULT_CMD): string {
  return `# ADE SDLC protocol
...
## Start of every task
0. If the project has no \`ade.config.json\`, run \`${cmd} init\` first.
1. Run \`${cmd} status\`.
...
export function pointerBlock(cmd = DEFAULT_CMD): string {
  return `${START}
# ADE SDLC protocol
This project follows the ADE protocol defined in AGENTS.md at the repository root. Read that block before any work and follow it exactly. Every task starts with \`${cmd} status\`; phases advance only with \`${cmd} next\`.
${END}
`;
}
