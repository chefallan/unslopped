...
export function describeGate(phase: Phase, config: Config): string {
  ...
  switch (phase) {
    case 'plan':
      return `${p.planSections.length ? `${p.planSections.join(', ')} filled in, ` : ''}${config.docs.length ? `${config.docs.length} project doc(s) present, ` : ''}${p.ladder ? 'the approach names a ladder rung, ' : ''}at least one acceptance criterion${p.criteriaQuality ? ', criteria are observable outcomes' : ''}`;
    case 'code':
