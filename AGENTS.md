...
2. No active cycle: run `npx unslopped start "<the user's request in one line>"`. Keep any issue key or link from the request (ENG-123, PROJ-45, #12, a Linear, Jira or GitHub URL) in that line. Unslopped links it, or the issue in the branch name, and copies its title and description into the plan. If start refuses because the branch is protected, either create the branch it suggests and start again, or run `npx unslopped start --worktree "<goal>"` to work in an isolated checkout and then `cd` into the path it prints. Every later command runs there. If start warns that uncommitted files predate the cycle, they belong in their own chore commit before this cycle's work, so the gates measure only this cycle. That commit is the human's: with no cycle active the hook refuses any commit of yours that stages a file outside `.unslopped/`, because there is no approved message for it. Ask, or start the cycle and put them through propose and approve inside it.
...
## How to report
End every task with `npx unslopped status` and a report the human can act on without reading the transcript.
