...
This project runs the DevOps lifecycle through ADE: plan, code, build, test, release, deploy, operate, monitor. Every phase has a gate that runs real commands. You cannot skip a gate and you cannot pass one by claiming success.
...
Questions you cannot answer go under `## Open questions` in the plan, not into invented answers. The release gate refuses to pass while any remain: get each answered by the human, record the answer, remove the question. A choice others must follow later (a boundary, a data ownership rule, a pattern) gets a durable record: `npx awesome-delivery-engine decide "<title>"`, fill it, commit it. Check `npx awesome-delivery-engine graph why` and docs/decisions/ before re-deciding something.

## Project tracker
ADE detects the tracker itself from env vars, project files, the git remote and MCP configs. You do not configure it. It comments on the linked issue and moves it at every phase change, so do not post tracker updates yourself. If a gate prints `WARN tracker: missing <VAR>`, tell the human which variable to set and keep working. Never paste credentials into any file.

## Phases
| Phase | Your job | Gate, checked by `npx awesome-delivery-engine next` |
|---|---|---|
| plan | Fill `.ade/plans/<id>.md`: goal, approach (how you will do it, which components change), out of scope, files to touch, acceptance criteria as a checklist. Decide before coding | Goal, approach and files to touch filled in, at least one acceptance criterion |
| code | Red before green: write the failing test first, run `npx awesome-delivery-engine red` and watch it fail, then implement the smallest change until it passes. Repeat per behaviour | Changes exist, source changes come with test changes, diff under the size limit, no secrets in the diff, lint passes |
...
| release | Verify each acceptance criterion and tick it `[x]` in the plan. Add a changelog entry if the project keeps one. Commit everything as `type(scope): imperative subject`, subject under 72 characters, scope naming the area touched. Then get the diff reviewed: `npx awesome-delivery-engine review` runs the configured reviewer, or a second agent writes findings as `[critical]`, `[major]`, `[minor]` lines and you submit them with `npx awesome-delivery-engine review --file=<path>`. Fix critical findings, commit, review again | Clean tree, new commits, every criterion ticked, commit messages match the pattern, changelog updated, no secrets, review newer than the last commit with no critical findings when enabled, human review approval when enabled, release command exits 0 |
| deploy | If the project is on GitHub, open the pull request first with `npx awesome-delivery-engine pr` (it pushes the branch and writes the PR body from the plan), then stop and tell the human to run `npx awesome-delivery-engine approve deploy`. Never run approve yourself. Never push protected branches | Approval recorded, rollback command configured, deploy command exits 0 |
| operate | Run the gate. Report the healthcheck output. If it fails, tell the human the rollback command. If a PR is open, `npx awesome-delivery-engine pr status` reports whether it merged and updates the tracker | Healthcheck exits 0 |
...
## Code map
ADE keeps a map of the codebase in `.ade/graph.json`: files, symbols, imports, importers, and the design rationale written in comments. It refreshes itself. Before reading or searching files, run `npx awesome-delivery-engine graph "<words>"` and read only the files it points to; the plan's `## Relevant code` section is filled from it. Before changing a shared file, run `npx awesome-delivery-engine graph why "<topic>"` to see the constraints already recorded, and `npx awesome-delivery-engine graph impact` to see who depends on what you touched and whether a test covers it. This is how you keep token use low: read the map, not the repository.

## Memory
ADE learns. Every completed cycle is saved as a skill in `.ade/skills/` without asking. Skills that match the goal are listed in the plan under `## Relevant skills`: read them first and follow the playbook instead of rediscovering it. A skill marked underperforming has failed too often; fix its `## Notes` section during the monitor phase so the next run goes better. When the user states a preference or corrects you, run `npx awesome-delivery-engine prefer "<one line>"` so it is remembered across sessions and projects. To find past work, decisions or failures, run `npx awesome-delivery-engine recall "<words>"`.
...
## Rules
- Advance only with `npx awesome-delivery-engine next`. If it fails, read the output, fix the cause, run it again.
...
- Out-of-scope problems you notice are logged, not fixed: `npx awesome-delivery-engine debt "<what>" --where=<path> --category=cleanup|pattern|soon|accepted`. One line, keep working.
- A completed cycle prints candidates for the next one, drawn from the monitor notes, the debt log and review leftovers. Offer them to the user; never start a cycle unasked. When the user wants the logged debt cleared, run `npx awesome-delivery-engine start --from-debt`; swept entries leave the log when that cycle completes.
...
- End every task with `npx awesome-delivery-engine status` and a short report: what changed, which gates ran, what is left.
<!-- ade:end -->
