...
function project({ cycle = false } = {}) {
  ...
  if (cycle) {
    fs.mkdirSync(path.join(dir, '.ade'), { recursive: true });
    fs.writeFileSync(path.join(dir, '.ade', 'state.json'), JSON.stringify({ cycle: { id: 'c1', goal: 'g', phase: 'code', history: [], approvals: {}, configHash: 'x' } }));
  }
  return dir;
}
...
test('session context carries the protocol, the status', () => {
  ...
  assert.match(text, /# ADE SDLC protocol/);
...
test('session context asks for init when the project has no config', () => {
  const dir = tmpDir();
  initRepo(dir);
  assert.match(sessionContext(dir), /not initialized. Run `ade init`/);
});
...
test('prompt context proposes a start command, the detected issue', () => {
  ...
  assert.match(text, /ade start "Fix the login timeout ENG-42"/);
...
test('prompt context asks for init on an uninitialized repo, is silent outside git', () => {
  const dir = tmpDir();
  initRepo(dir);
  assert.match(promptContext(dir, 'x'), /Run `ade init` first/);
...
test('tool decisions block human only commands, state edits', () => {
  ...
  assert.equal(bash('ade approve deploy').block, true);
  assert.equal(bash('npx awesome-delivery-engine reset').block, true);
  ...
  assert.equal(bash('echo {} > .ade/state.json').block, true);
  assert.equal(bash('rm -rf .ade/cycles').block, true);
  assert.equal(bash('cat .ade/state.json').block, false);
  assert.equal(bash('ade status --json').block, false);
  assert.equal(bash('sed -i s/a/b/ ade.config.json').block, true);
  assert.equal(bash('cat ade.config.json').block, false);
  assert.equal(bash('git push origin main').block, false);
  assert.equal(bash('npm test').block, false);
  assert.equal(bash('ade next').block, false);
});

test('tool decisions block edits to state and, mid cycle, to config', () => {
  const active = project({ cycle: true });
  assert.equal(toolDecision(active, 'Edit', { file_path: path.join(active, '.ade', 'state.json') }).block, true);
  assert.equal(toolDecision(active, 'Write', { file_path: path.join(active, '.ade', 'cycles', 'x.json') }).block, true);
  assert.equal(toolDecision(active, 'Write', { file_path: path.join(active, 'ade.config.json') }).block, true);
  assert.equal(toolDecision(active, 'Edit', { file_path: path.join(active, 'src', 'a.js') }).block, false);
  assert.equal(toolDecision(active, 'Edit', { file_path: path.join(active, '.ade', 'plans', 'c1.md') }).block, false);
  const idle = project();
  assert.equal(toolDecision(idle, 'Write', { file_path: path.join(idle, 'ade.config.json') }).block, false);
  assert.equal(toolDecision(idle, 'Bash', { command: 'echo x > ade.config.json' }).block, false);
});

test('tool decisions stay out of projects that do not use ade', () => {
  const dir = tmpDir();
  initRepo(dir);
  assert.equal(toolDecision(dir, 'Bash', { command: 'git push --force' }).block, false);
  assert.equal(toolDecision(dir, 'Bash', { command: 'ade approve deploy' }).block, false);
});
