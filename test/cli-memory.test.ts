...
function runner(home) {
  return (cwd, ...args) => {
    ...
    const r = spawnSync(process.execPath, [BIN, ...args], { cwd, encoding: 'utf8', input, env: { ...process.env, ...NO_TRACKER_ENV, ADE_HOME: home } });
...
test('completed cycles become skills that later cycles reuse', () => {
  ...
  const skill = path.join(dir, '.ade', 'skills', 'add-health-endpoint.md');
  ...
  assert.equal(r.code, 0);
  assert.match(fs.readFileSync(path.join(dir, '.ade', 'skills', 'rotate-secrets.md'), 'utf8'), /use the vault cli/);
...
test('prefer, profile, recall work across the global home', () => {
  ...
  assert.ok(fs.existsSync(path.join(home, '.ade', 'profile.md')));
...
test('the prompt hook records history, surfaces skills, preferences', () => {
  ...
  const history = fs.readFileSync(path.join(dir, '.ade', 'history.jsonl'), 'utf8').trim().split('\n').map((l) => JSON.parse(l));
